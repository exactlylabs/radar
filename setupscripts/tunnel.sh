...
do
  ...
  TUNNEL_CONFIG=$(curl -H "Accept: application/json" -XPOST $API_ENDPOINT_URL/clients/$CLIENT_ID/configuration -d "secret=$CLIENT_SECRET")

  PRIVATE_KEY=$(echo $TUNNEL_CONFIG | jq -r .private_key)
  ENDPOINT_HOST=$(echo $TUNNEL_CONFIG | jq -r .endpoint_host)
  ENDPOINT_PORT=$(echo $TUNNEL_CONFIG | jq -r .endpoint_port)
  REMOTE_GATEWAY_PORT=$(echo $TUNNEL_CONFIG | jq -r .remote_gateway_port)
