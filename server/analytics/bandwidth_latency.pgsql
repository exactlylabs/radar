WITH location_grouped AS (
  SELECT
    ...
    percentile_disc(0.5) WITHIN GROUP (ORDER BY measurements.latency) as "latency_median",
    avg(measurements.latency) AS "average",
    stddev(measurements.latency) as "stdd",
    location_id
...
SELECT 
  time,
  percentile_disc(0.5) WITHIN GROUP (ORDER BY latency_median) as "latency_median",
