WITH location_grouped AS (
  SELECT
    ...
    percentile_disc(0.5) WITHIN GROUP (ORDER BY measurements.download) as "download_median",
    avg(measurements.download) AS "average",
    stddev(measurements.download) as "stdd",
    location_id
