with base_series AS (
  ...
  LEFT JOIN accounts ON aggregator_id = accounts.id AND aggregator_type = 'Account'
  LEFT JOIN locations ON aggregator_id = locations.id AND aggregator_type = 'Location'
  LEFT JOIN autonomous_systems ON autonomous_systems.id = aggregator_id AND aggregator_type = 'AutonomousSystem'
  LEFT JOIN autonomous_system_orgs ON autonomous_system_orgs.id = autonomous_systems.autonomous_system_org_id
  WHERE
    CASE WHEN '$dimension' = 'account' THEN
      aggregator_id IN ($entities) AND aggregator_type = 'Account'
    WHEN '$dimension' = 'isp' THEN
      aggregator_id IN ($entities) AND aggregator_type = 'AutonomousSystem'
    WHEN '$dimension' = 'location' THEN
      aggregator_id IN ($entities) AND aggregator_type = 'Location'
    END
), initial_state AS (
  SELECT
  ...
  SELECT aggregator_id, name, "time", online
  FROM filtered_dimensions
