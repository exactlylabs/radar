...
export default class extends Controller {
  ...
  connect() {
    // Initial empty state (Location creation)
    if (!this.nameTarget.value) {
      fetch("/geocode", { method: "POST" })
        .then((res) => res.json())
        .then((res) => {
          // Mainly a safe net in case the geocoder fails and returns NaN then we can show the pin.
          // Without validation the map would be empty (no pin even)
          if (isNaN(res[0]) || isNaN(res[1])) res = [0, 0];
