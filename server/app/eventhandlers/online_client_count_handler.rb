class OnlineClientCountHandler
  ...
  def handle_client_event!(event)
    ...
    last_count = OnlineClientCountProjection.latest_for state["account_id"], state["autonomous_system_id"], state["location_id"]
    if last_count.nil?
    ...
    when Client::Events::ACCOUNT_CHANGED
      if state["online"]
        old_account_count = OnlineClientCountProjection.latest_for event.data["from"], state["autonomous_system_id"], state["location_id"]
        self.new_record!(last_count, event, increment=1)
    ...
    when Client::Events::LOCATION_CHANGED
      if state["online"]
        old_location_count = OnlineClientCountProjection.latest_for state["account_id"], state["autonomous_system_id"], event.data["from"]
        self.new_record!(last_count, event, increment=1)
    ...
    when Client::Events::AS_CHANGED
      if state["online"]
        old_as_count = OnlineClientCountProjection.latest_for state["account_id"], event.data["from"], state["location_id"]
        self.new_record!(last_count, event, increment=1)
