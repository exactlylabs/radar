...
class FillClientCountProjection < ApplicationJob
  ...
  around_enqueue do |_job, block|
    ...
    Sidekiq::Queue.new("study_projection").each do |j|
      if j.args[0]["job_class"] = "FillClientCountProjection"
        already_enqueued = true
      end
    end
    Sidekiq::WorkSet.new.each do |process_id, thread_id, work|
      if JSON.parse(work["payload"])["args"][0]["job_class"] == "FillClientCountProjection"
        already_enqueued = true
      end
    end
