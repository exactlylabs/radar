...
export function middleware(request: NextRequest) {
  ...
  if (MATCHERS.TBP.test(pathName)) {
    ...
    setSourceAndDateForCampaign(queryParams, MATCHERS.GO_ALASKA.SOURCE, MATCHERS.GO_ALASKA.DATE);
  } else if (MATCHERS.GO_MICHIGAN.URL.test(pathName)) {
    nextUrl = '/GO';
    queryParams.append('state',MATCHERS.GO_MICHIGAN.NAME);
    setSourceAndDateForCampaign(queryParams, MATCHERS.GO_MICHIGAN.SOURCE, MATCHERS.GO_MICHIGAN.DATE);
  } else if (MATCHERS.GO_TEXAS.URL.test(pathName)) {
    nextUrl = '/GO';
    queryParams.append('state',MATCHERS.GO_TEXAS.NAME);
    setSourceAndDateForCampaign(queryParams, MATCHERS.GO_TEXAS.SOURCE, MATCHERS.GO_TEXAS.DATE);
  } else if (MATCHERS.GO_WEST_VIRGINIA.URL.test(pathName)) {
    nextUrl = '/GO';
    queryParams.append('state',MATCHERS.GO_WEST_VIRGINIA.NAME);
    setSourceAndDateForCampaign(queryParams, MATCHERS.GO_WEST_VIRGINIA.SOURCE, MATCHERS.GO_WEST_VIRGINIA.DATE);
  }
...
function setSourceAndDateForCampaign(queryParams: URLSearchParams, source: string, date: string) {
