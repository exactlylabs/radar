CREATE MATERIALIZED VIEW summary_geospace_quarter_tmp
ENGINE=MergeTree
ORDER BY (upload, geospace_id, year, quarter)
POPULATE
AS
SELECT
...
FROM (
    ...
    FROM measurements m
    GROUP BY m.geospace_id, m.upload, m.ip, year, quarter
) m_ip 
GROUP BY m_ip.geospace_id, m_ip.upload, m_ip.year, m_ip.quarter;