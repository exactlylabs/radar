...
ENGINE=MergeTree
ORDER BY (geospace_id, asn_id, year, semester)
POPULATE
AS
SELECT
...
FROM (
    ...
    FROM measurements m
    GROUP BY m.geospace_id, m.asn_id, m.ip, year, semester
) m_ip 
GROUP BY m_ip.geospace_id, m_ip.asn_id, m_ip.year, m_ip.semester;