...
func ServeMultiLayeredVectorTiles(c *webcontext.Context) {
	...
	conf := c.MustGetValue("config").(*config.Config)
	servers := c.MustGetValue("tilesetServers").(*geo.TilesetServers)
	vectorTileServer := servers.MultiLayered
	summaries := c.MustGetValue("summariesStorage").(storages.SummariesStorage)
	args := &ServeVectorTilesArgs{}
	args.Parse(c)
	...
	fmt.Println("Time for validation:", time.Since(start))
	start = time.Now()
	vt, err := vectorTileServer.Get(x, y, z)
	if err == geo.ErrEmptyTile {
		return
	} else if err != nil {
		panic(errors.Wrap(err, "routes.ServeMultiLayeredVectorTiles GetVectorTile"))
	}
	if vt == nil {
		return
	}
	...
	fmt.Println("Time to load summary results:", time.Since(start))
	start = time.Now()
	vt.Range(func(layer namespaces.Namespace, feature geo.Feature) (bool, error) {
		geospaceId := feature.GetProperties()["ID"].(string)
		if summary, exists := nsSummary[layer][geospaceId]; exists {
			// If passing summary directly, the library fails to encode it
			// we need to pass a non-comparable type to it
			feature.SetProperty("summary", map[string]any{
				"geospace":        summary.Geospace,
				"asn":             summary.ASNOrg,
				"download_median": summary.DownloadMedian,
				"upload_median":   summary.UploadMedian,
